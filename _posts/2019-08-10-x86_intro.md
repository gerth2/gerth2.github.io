...
```nasm
{% fromFile _samples/sample_mov_asm/sample.asm %} 
```
...
```nasm
{% fromFile _samples/sample_math_asm/sample.asm %}
```
...
```nasm
{% fromFile _samples/sample_logic_asm/sample.asm %}
```
...
```nasm
{% fromFile _samples/sample_jmp_asm/sample.asm %}
```
...
With the end of this post, hopefully we've presented a decent introduction to the bare-bones components of x86, a real assembly language that works on real processors. We'll use this knowledge to help deconstruct how our high level language (C code) gets converted to 1's and 0's on a processor, with x86 assembly as the intermediate description of the bits. Coming soon!
