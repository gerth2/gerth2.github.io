...
For _real control system_ work, you'll want your relationships to by quantifiable.  It could be a command communicated to a motor controller, a force applied from one mechanism to another, a position detected by a sensor, an electrical signal generated by a sensor and sent to a controller... you get the picture. The key is _quantitative_ - arrows are like assignment in variables, and actually get interpreted as such when you write software or do math based in block diagrams.
...
2. We specify that our _Plant_ is made up of an electrically-powered actuator, some physical mechanism attached to that actuator, and a sensor measuring that mechansim. 
...
So far, we're on step 1. We're building up a language and technique for describing the problem.

We've described how control systems are broken down into components, represented with nice little boxes. The next major task is to start to fill out the contents of each box.
...
[^1]: Backward, assuming you read from left to right. Sorry Hebrew friends.