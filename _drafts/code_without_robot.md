...
date:   2020-11-06 09:30:40 -0500
categories: blog_posts
---

# Hello Again

Six months off? Whoops.
...
[Here it is, if you're curious](https://github.com/RobotCasserole1736/OffseasonTraining). It's on our team's github for two reasons:
...
As we started to put that training together, it became very quickly relevant to have the ability to develop robot code fully in simulation. Based on that and the lessons learned, I thought it might be worthwhile to put down some thoughts on the topic.
...
_Simulation is the set of enabling technologies to let you run a set of source code in some alternate environment_.

I most often hear it used to refer to embedded code development where the real hardware that ordinarily runs and interacts with the code isn't present. Though conceptually similar to more "pure-software" environments (websites, phone apps, etc.), "emulation" or "development/production" nomenclature is more commonly used.

There's a couple different aspects to these various _enablers_. One portion is getting the code running on different processing hardware than where it usually runs. Another is picking which pieces of code will be "in scope" for simulation, and which ones you'll have to "stub", making simulation-specific alternates. Finally, there may need to be some new code written to roughly emulate the behavior of hardware which no longer exists.
...
This is a good example of why standards are important. You don't want a single-purpose programming language - you want source code which can be interpreted by many different compiler toolchains, which in turn enables the toolchains to target different computer & hardware sets. However, _assuming_ you stick to the standard and use common languages in FRC, you'll very likely be set.

On top of all this, WPILib has been very good recently about ensuring that libraries (including those from vendors) can at least build for simulation on a desktop PC. You'll want to follow their instructions for how to ensure your gradle setup is functional for simulation.
...
The most obvious one, which most folks bring up right away - the ability to "test" your code and "write code without a robot". These are certainly true, but also deserve a bit of nuance of discussion.

Even without any simulation infrastructure, you can of course _write_ new code. The add of simulation allows you to also _build_ and (in some way) _run_ the robot code. This is indeed an extra step beyond just writing new lines, and will give a good bump in the quality of the code you put out right away. 

However, in and of itself, it's not a panacea for all issues - it doesn't prevent you from having bugs in your code. Nor does it mean that the software team suddenly doesn't need any development time on the real robot.

These points might seem obvious, but you have to be careful how you sell "simulation" to other people on the team. I've seen many a time when the abilities of a simulation environment were poorly communicated, folks came in with very different expectations, and weeks later a great level of disappointment had happened.

Still, this isn't to back away from the positives. Simply letting your code run is a pretty decent way to find basic mistakes like null pointer dereferences, uninitialized objects, unexpected recursion.... add in just enough to let you enable in Teleop and move some joysticks, and you'll be able to actually test lots of your code before hitting the robot. That in and of itself makes it worthwhile for my teams, as time on the robot is very valuable and not very abundant.
...
Still, it's pretty hard: There also isn't any commonly-supported software suite I'm aware of. Still, hopefully, one of these days....
...
Will you be using your test results to pass information back to the mechanical or other design teams? Or are they just to validate the software has some sort of functionality?
...
What tools and infrastructure has been provided already? How much time are you willing to invest in something bespoke?

Is there already some build infrastructure set up for an alternate environment? Is there a virtual machine setup available? How accessible is the available content - can you edit and modify easily to your own needs, or is it very rigid?
